...
ขอบเขตของชนิดข้อมูลสำคัญมากโดยเฉพาะ int

ต้องระมัดระวังเมื่อทำงานกับค่าที่ใกล้ขอบเขตมาก

หากต้องการจัดการค่าที่ใหญ่กว่านี้ → ควรใช้ long, int64_t, หรือ unsigned int แทน
...
- float และ double บน ESP32 ใช้กี่ไบต์ตามลำดับ?
  ...
  floatใช้4ไบต์ และdoubleใช้8ไบต์

- นักศึกษาสังเกตเห็นความแตกต่างของความแม่นยำระหว่าง float และ double อย่างไร? สถานการณ์ใดที่คุณควรเลือกใช้ double แทน float?

  บนแพลตฟอร์มทั่วไปถ้าต้องการความแม่นยำสูงขึ้น (ประมาณ 15-16 หลักทศนิยม) ควรใช้ double เพราะให้ precision ที่สูงกว่า

  บน ESP32 หรือแพลตฟอร์มที่doubleเป็น4bytesเหมือนfloat การใช้doubleไม่ได้เพิ่มความแม่นยำควรใช้floatเพราะประหยัดทรัพยากรและdoubleไม่มีข้อดีเรื่องความแม่นยำ
...
- char ใช้กี่ไบต์? ค่าตัวเลข (ASCII value) มีความสัมพันธ์กับอักขระอย่างไร?
  ...
  char เป็นตัวเก็บรหัสตัวอักษรในรูปแบบเลข ASCII โดยใช้พื้นที่ 1 ไบต์ และค่าตัวเลขนั้นแทนแต่ละอักขระตามมาตรฐาน ASCII
...
- bool ใช้กี่ไบต์? true และ false ถูกแสดงผลเป็นค่าใดบน Serial Monitor?
  ...
  bool ใช้พื้นที่ 1 ไบต์

  ค่า true แสดงผลเป็นเลข 1

  ค่า false แสดงผลเป็นเลข 0 
...
- ชนิดข้อมูลจำนวนเต็มแต่ละชนิด (long, long long, unsigned int, unsigned long, unsigned long long) ใช้กี่ไบต์บน ESP32?
  ...
  long และ unsigned long เป็น 4 ไบต์

  long long และ unsigned long long เป็น 8 ไบต์

  unsigned int บน ESP32 ก็ 4 ไบต์ เหมือน unsigned long

- บน ESP32, long มีขอบเขตเท่ากับ int หรือไม่? ชนิดข้อมูลใดที่คุณจะใช้หากต้องการเก็บค่าจำนวนเต็มบวกที่ใหญ่ที่สุด?

  long มีขนาดเท่ากับ int คือ 4 ไบต์ ถ้าต้องการเก็บค่าจำนวนเต็มบวกที่ใหญ่ที่สุดควรใช้ชนิดข้อมูล unsigned long long 
...
byte ใช้ 1 ไบต์ เมื่อกำหนดค่า myByte = 256 ซึ่งเกินขอบเขตที่เก็บได้ ค่าเกินจะถูกตัดส่วนที่เกินออก ผลลัพธ์ที่ได้คือ 0
...
เมื่อตัวหารเป็น float หรือ double การคำนวณจะเป็นการหารแบบเลขทศนิยม(floating-point division)

แต่เมื่อแปลงตัวหารเป็น int การคำนวณจะเป็นการหารแบบจำนวนเต็ม(integer division)
...
เปลี่ยนชนิดข้อมูลเพื่อการคำนวณที่ถูกต้อง

แปลงข้อมูลระหว่างชนิดข้อมูลที่ไม่ตรงกัน

ปรับแต่งประสิทธิภาพและขนาดข้อมูล
